...
def load_documents(dir_path="data_demo", file_path="./data_demo/constitutionofindiaacts.pdf", save_md_file=True, use_pymup=True):
    if use_pymup:
        if save_md_file:
...
if __name__ == "__main__":
    ...
    Settings.llm = Ollama(model="llama3.2",request_timeout=360.0, temperature=0.4,context_window=4096)
    # response = llm.complete("why sky is blue")
    # print(response)
    if os.path.isdir("index_store"):
        index = load_from_storage()
    else:
        docs = load_documents(dir_path="data_demo",use_pymup=False)
    ...
    # response = query_llm("Summary of the preamble of indian constituion?", index)
    # print(response)
